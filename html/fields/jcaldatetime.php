...
class JFormFieldJCalDateTime extends JFormField
{
	...
	protected function getInput() {
        ...
        $hour = '<div class="col-sm-2">'.$hour.'</div>';
        $minute = '<div class="col-sm-2">'.$minute.'</div>';

		$hourminute = $hour . $minute;

        $day = '<div class="col-sm-2">'.$day.'</div>';
        $month = '<div class="col-sm-3">'.$month.'</div>';
        $year = '<div class="col-sm-2">'.$year.'</div>';

		return '<div class="row">' . $day . $month . $year . (empty($hourminute) ? '' : '<div class="col-sm-1 jcaldatetime_at">' . JText::_('COM_JCALPRO_DATETIME_AT')) . '</div>' . $hourminute . '</div>';
	}
